...
import { UnitWithName, API_BASE_URL } from '../../../../shared/clients';
...
export class SpyingPageComponent implements OnInit {
  ...
  constructor(
    private spyingService: SpyingService,
  ...
  private getAllData() {
    ...
    forkJoin(
      ...
      const explorerDetails = unitDetails.find(
        (ud) => ud.name === 'Felfedező'
      )!;
      const countryExplorers = countryUnits.find(
        (cu) => cu.id === explorerDetails.id
      );
      const countryExplorersCount = countryExplorers?.count ?? 0;
      const unitsToSubtract = this.getNumberOfUnitsWhoAreInBattle(
        sentSpiesDetails
      ).count;
  ...
  private updateCountryUnits() {
    this.spyingService.getCountryUnits().subscribe((cus) => {
      if (this.explorerUnits[0]) {
        const countryExplorers = cus.find(
          (cu) => cu.id === this.explorerUnits[0].id
        ) ?? {
          count: 0,
        };
  ...
  private getNumberOfUnitsWhoAreInBattle(
    sentSpiesDetails: Battle[]
  ): UnitWithName {
    ...
    sentSpiesDetails.forEach((cb) => {
      const num =
        cb.units.find((unit) => unit.name === 'Felfedező')?.count ?? 0;
      result.count += num;
    });
    return result;
  }
  ...
  onExplore() {
    this.spyingService
      .explore({
        targetCountryId: this.selectedPlayerId,
        numberOfExplorers: this.explorerUnits[0].countToAttack,
      })
      .subscribe(() => {
  ...
  isReadyToSend(): boolean {
    return (
      this.selectedPlayerId !== -1 && this.explorerUnits[0].countToAttack > 0
    );
  }
}
