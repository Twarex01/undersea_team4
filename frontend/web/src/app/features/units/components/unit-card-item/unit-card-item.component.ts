...
export class UnitCardItemComponent implements OnInit {
  ...
  canIncrease(): boolean {
    if (
      this.unitBuyInfo.armyCapacity <= this.unitBuyInfo.estimatedUnitsCount ||
      this.unitBuyInfo.estimatedPearlCost + this.unit.price >
        this.unitBuyInfo.pearl
    ) {
      return false;
    }
    return true;
  ...
  changeNumberOfUnitsToBuy(num: number) {
    if (this.unit.numToBuy === 0 && num < 0) return;
